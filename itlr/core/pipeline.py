"""Phễu xếp hạng nhiều tầng (multi-stage ranking).
...
BẬT/TẮT độc lập từng tầng (phục vụ ablation + đo chi phí–chất lượng từng tầng):
  ...
  Stage 2 — L2 Re-ranking (nặng) : Cross-Encoder rerank (hoặc Learning-to-Rank).
...
class RankContext:
    ...
    ltr_scorer: Optional[Callable] = None
    cf_popularity: object = None
...
class StageConfig:
    ...
    name: str = "full"
    candidate_source: str = "embedding"
    l1_signals: frozenset = field(default_factory=frozenset)
...
def _rag_fusion_candidates(ctx: RankContext, query: str, cfg: StageConfig) -> List[int]:
    ...
    fused = R.reciprocal_rank_fusion(rankings)
