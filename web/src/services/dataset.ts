...
// Trích nội dung text từ PDF / DOCX / Excel. Dùng dynamic import (chỉ nạp lib khi cần,
// và tránh self-test của pdf-parse khi import tĩnh ở ESM). Lỗi -> trả "" (không chặn upload).
export async function extractText(buf: Buffer, ext: string): Promise<string> {
  try {
    if (ext === ".pdf") {
      // @ts-ignore - import thẳng lib để bỏ qua debug wrapper của pdf-parse
...
// Gộp khoảng trắng + cắt -> 1 dòng, an toàn để ghi vào CSV (không xuống dòng).
export function cleanForCsv(text: string, max = 3000): string {
  return (text || "").replace(/\s+/g, " ").trim().slice(0, max);
}

// ── Nạp CỘNG DỒN vào data/it_learning_items.csv (giữ nguyên 10 cột) ─────────────
...
function readLastId(): number {
  ...
  for (let i = lines.length - 1; i >= 1; i--) {
    const n = parseInt(lines[i].split(",")[0], 10); // cột item_id luôn là số ở đầu dòng
...
// Thêm 1 dòng vào CSV (1 dòng = 1 bản ghi). Trả item_id mới. Lỗi ghi -> ném để caller bắt.
